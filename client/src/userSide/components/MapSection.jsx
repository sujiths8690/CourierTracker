...
const normalizePoint = (point) =>
  hasValidPoint(point) ? point.map((value) => Number(value)) : null;
...
export default function MapSection({
  ...
  function SmartFit({ pos, route, destinationRoute, destination }) {
    ...
    useEffect(() => {
      const currentPoint = normalizePoint(pos);
      ...
      if (currentPoint && destinationPoint) {
        map.fitBounds([currentPoint, destinationPoint], {
          padding: [70, 70],
          maxZoom: 13
        });
        return;
      ...
      if (destinationPoint) {
        map.setView(destinationPoint, 13);
        return;
      }
