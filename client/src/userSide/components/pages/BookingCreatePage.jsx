...
export default function BookingCreatePage({ setPage, t }) {
  ...
  const handleSearch = async (query) => {
    ...
    useEffect(() => {
        ...
        const loadNearbyVehicles = (location) => {
  ...
  const vehicleLookupLocation = useMemo(() => {
  ...
  }, [currentLocation, form.pickupLat, form.pickupLng]);

  useEffect(() => {
    if (!vehicleLookupLocation) return;

    const refreshNearbyVehicles = () => {
      dispatch(fetchNearbyVehicles({
        ...vehicleLookupLocation,
        radius: 20
      }));
    };
    ...
    const interval = setInterval(refreshNearbyVehicles, 10000);

    return () => clearInterval(interval);
  }, [dispatch, vehicleLookupLocation]);
